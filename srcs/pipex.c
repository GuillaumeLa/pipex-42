...
void child_proc(char **env , char **argv,int pipe_fds[2])
{
...
void parent_proc(char **env , char **argv,int pipe_fds[2])
{
...
int main(int argc,char **argv,char **env)
{
	...
	if(pid == -1)
		return(2);
	if(pid == 0)
		child_proc(env,argv,pipe_fds);
	else
		parent_proc(env,argv,pipe_fds);
